...
const Home = () => {
  ...
  const refreshFeed = useCallback(async () => {
    ...
    try {
      ...
      const previousCachedPostIds = new Set(
      ...
      if (nfRes.status === 200) {
        const latestPosts = Array.isArray(nfRes.data.posts)
          ? nfRes.data.posts
          : [];
        const newPostsInFetch = isFirstLoadRef.current
          ? []
          : latestPosts.filter(
              (post: any) => post?._id && !previousCachedPostIds.has(post._id),
            );
      ...
      const errorMessage =
        e?.response?.data?.message || 'Failed to load posts. Please try again.';
      ...
      <View>
        ...
        {showNewPostsNotification && (
          ...
          >
            ...
            >
              🆕 New Posts! {newPostsCount} new{' '}
              {newPostsCount === 1 ? 'post' : 'posts'} available
            </Text>
            <TouchableOpacity
              onPress={() => setShowNewPostsNotification(false)}
              hitSlop={8}
            >
  ...
  const listFooterComponent = useMemo(
    () =>
      loadingMore && uniquePosts.length > 0 ? (
