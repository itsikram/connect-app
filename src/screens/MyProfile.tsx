...
const MyProfile = () => {
  ...
  const [showCoverCropModal, setShowCoverCropModal] =
    React.useState<boolean>(false);
  ...
  const fetchProfileData = React.useCallback(async () => {
    ...
    const profileId = String(myProfile._id);

    // Posts (for Posts and Images tabs)
    setPostsLoading(true);
    api
      .get('/post/myPosts', { params: { profile: myProfile._id } })
      .then(res => {
    ...
    api
      .get('/watch/profileWatch', {
        params: { profile: myProfile._id, pageNumber: 1 },
      })
      .then(res => {
        if (res.status === 200 && String(myProfile._id) === profileId) {
  ...
  const handleProfileImageSelected = async (uri: string) => {
    ...
    try {
      ...
      const res = await api.post('/profile/update/profilePic', {
  ...
  const tabs: {
      ...
      render: () => (
        <View style={{ gap: 10 }}>
          ...
          >
            ...
            {Array.isArray(myProfile?.workPlaces) &&
              myProfile.workPlaces.map((wp: any, idx: number) => (
                <View key={`wp-${idx}`} style={styles.detailsItem}>
                  ...
                  >
                    ...
                    >
                      {wp?.name || 'Unknown workplace'}
                    </Text>
                  </Text>
                </View>
              ))}

            {Array.isArray(myProfile?.schools) &&
              myProfile.schools.map((sc: any, idx: number) => (
            ...
            <View style={styles.detailsItem}>
              ...
              >
                ...
                >
                  {formatMonthYear(
                    myProfile?.user?.createdAt || myProfile?.createdAt,
                  )}
                </Text>
              </Text>
            </View>
            ...
            <View style={styles.friendsGrid}>
              {friends.map((f: any) => {
                const userName =
                  f.fullName ||
                  (f.user
                    ? `${f.user.firstName || ''} ${f.user.surname || ''}`.trim()
                    : f.username) ||
                  'Unknown';
            ...
            <View style={styles.viewerOverlay}>
              ...
              <View style={styles.viewerContent}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    setImageViewerIndex(i => Math.min(images.length - 1, i + 1))
                  }
                >
                  <Icon
                    name="chevron-right"
    ...
    },
    {
      ...
      render: () => (
        <View>
          {videosLoading && <ProfileMediaSkeleton count={2} />}
              ...
              {videos.map((v: any) => {
                return (
                  <ProfileVideoCard
                    ...
                    onOpen={() =>
                      (navigation as any).navigate('SingleWatch', {
                        watchId: v._id,
                      })
                    }
                    onDeleted={watchId =>
  ...
  React.useEffect(() => {
    ...
    api
      .get('/profile/getImages', { params: { profileId: myProfile._id } })
      .then(res => {
      ...
      >
        ...
        <View style={styles.tabRow}>
          <ScrollView
            ...
            contentContainerStyle={styles.tabNavigator}
          >
