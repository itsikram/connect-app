...
import {
  ...
  ChatPageSkeleton,
} from '../components/skeleton/ChatSkeleton';
...
import {
  playSpeakPayload,
  stopSpokenPlayback,
} from '../lib/speakMessagePlayback';
import {
  createPlayableVoiceSound,
  isAudioAttachmentUrl,
} from '../lib/voiceMessageAudio';
...
const normalizeMessageReactions = (reacts: any[] = []): MessageReaction[] =>
  (Array.isArray(reacts) ? reacts : []).reduce(
    (result: MessageReaction[], reaction: any) => {
      const profile = reaction?.profile || reaction;
      const profileId = String(profile?._id || profile || '');
      if (
        !profileId ||
        result.some(item => reactionProfileId(item) === profileId)
      ) {
        return result;
      }
...
const reactionProfileId = (reaction: MessageReaction) =>
  String(
    typeof reaction?.profile === 'object'
      ? reaction.profile?._id || ''
      : reaction?.profile || '',
  );
...
const getMessageSnippet = (msg: any) => {
  ...
  if (msg?.messageType === 'audio' || isAudioUrl(msg?.attachment || ''))
    return 'Voice message';
  if (typeof msg?.attachment === 'string' && isValidImageUrl(msg.attachment))
    return 'Photo';
  return 'Message';
};
...
const SingleMessage = () => {
  ...
  const shareFaceModeEnabled = settings.isShareEmotion === true;
  const shareFaceModeEnabledRef = useRef(shareFaceModeEnabled);
  shareFaceModeEnabledRef.current = shareFaceModeEnabled;
  ...
  const [pendingAttachmentLocal, setPendingAttachmentLocal] = useState<
    string | null
  >(null);
  ...
  const stopTranscriptionRef = useRef<
    (opts?: { discard?: boolean }) => void | Promise<void>
  >(() => {});
  const sendVoiceMessageRef = useRef<(url: string, duration: number) => void>(
    () => {},
  );
  ...
  // expo-camera has no device-discovery hook. Permission and CameraView
  // readiness determine availability, while `facing` selects the camera.
  const cameraDevice = EXPO_CAMERA_DEVICE;
  ...
  const stopRecording = async (shouldSend: boolean) => {
    ...
    const recordedDuration = Math.max(recordSecs, Math.floor(elapsedMs / 1000));
    ...
    try {
      ...
      if (shouldSend && uri) {
        if (elapsedMs < 400) {
          Alert.alert(
            'Voice message',
            'Hold a bit longer to record a voice message.',
          );
          return;
        }
        await uploadAndSendAudio(uri, recordedDuration);
      }
    } catch (e) {
  ...
  const uploadAndSendAudio = async (filePath: string, duration: number) => {
    try {
      ...
      if (voiceUrl) {
        sendVoiceMessageRef.current?.(voiceUrl, duration);
      } else {
  ...
  const seekTo = async (item: Message, seconds: number) => {
    setPlayingProgress(prev => ({
      ...
      [item._id]: {
        current: seconds,
        duration: prev[item._id]?.duration || item.duration || 0,
      },
    }));
  ...
  const emotionDetectionIntervalRef = React.useRef<ReturnType<
    typeof setInterval
  > | null>(null);
  ...
  const handleEmotionServerResponseRef = React.useRef<
    ((data: any) => void) | null
  >(null);
  const cameraSetupTimeoutRef = React.useRef<ReturnType<
    typeof setTimeout
  > | null>(null);
  ...
  const SERVER_REQUEST_TIMEOUT_MS = 30000; // FER may take longer on the first frame
  const PERMISSION_CHECK_COOLDOWN_MS = 10000; // Only check permission every 10 seconds

  const logDetectionProgress = (
    event: string,
    details: Record<string, unknown> = {},
  ) => {
    const stats = detectionStatsRef.current;
    const now = Date.now();
    if (now - stats.lastProgressLogAt < 5000 && event === 'interval') return;
    stats.lastProgressLogAt = now;
    console.log('[SingleMessage] 🔎 Detection progress:', {
      event,
      ...details,
      cameraRef: !!cameraRef.current,
      cameraReady: isCameraReadyRef.current,
      permission: isCameraPermissionGranted,
      device: !!cameraDevice,
      socketConnected: !!emotionServerSocketRef.current?.connected,
      requestInFlight: serverRequestInFlightRef.current,
      ...stats,
    });
  };
  ...
  const loadMessagesFromStorage = async (
    ...
    try {
      ...
      const deserializedMessages: Message[] = parsedMessages.map(
        (msg: any) => ({
          ...msg,
          timestamp: msg.timestamp ? new Date(msg.timestamp) : new Date(),
        }),
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (messageData: any) => {
      ...
      const nextMessages = upsertConfirmedMessage(
        messagesRef.current,
        newMessage,
        newMessage.tempId,
      );
    ...
    const handleReceiveTyping = (typingData: any) => {
      ...
      if (typingData?.isTyping) {
        setIsTyping(true);
        setTypingMessage(
          typeof typingData.type === 'string' ? typingData.type : '',
        );
    ...
    const handleReactionUpdate = (payload: any) => {
      ...
      const reacts = normalizeMessageReactions(
        payload?.reactions || updated.reacts,
      );
      setMessages(prevMessages =>
  ...
  useEffect(() => {
    ...
    console.log('[SingleMessage] 📊 Current state:', {
      isShareEmotion: shareFaceModeEnabled,
    ...
    if (!shareFaceModeEnabled) {
      console.log(
        '[SingleMessage] ⏸️ Emotion detection disabled - face share mode is off',
      );
      return;
    }
    ...
    if (!currentProfileId) {
      console.log(
        '[SingleMessage] ⏸️ Emotion detection disabled - no profileId',
      );
    ...
    if (isCallActive) {
      console.log(
        '[SingleMessage] ⏸️ Emotion detection disabled - call is active',
      );
    ...
    console.log(
      '[SingleMessage] ✅ All conditions met, proceeding with emotion detection setup',
    );
    ...
    console.log(
      '[SingleMessage] ✅ IDs validated, starting emotion detection initialization',
    );
    console.log(
    ...
    const initializeEmotionServerSocket = async () => {
      ...
      try {
        ...
        try {
          ...
          console.warn(
            '[SingleMessage] ⚠️ Could not resolve live face service URL:',
            error?.message || error,
          );
        }
        if (!pythonServerUrl) {
        ...
        if (!pythonServerUrl) {
          console.warn(
            '[SingleMessage] ⚠️ Face service URL is unavailable from the face-login server',
          );
          return;
        }
        ...
        console.log(
          '[SingleMessage] 🔌 Connecting to Python emotion detection server:',
          pythonServerUrl,
        );
        logDetectionProgress('socket_connecting', {
          url: pythonServerUrl,
        ...
        emotionServerSocketRef.current = io(`${pythonServerUrl}/expressions`, {
          ...
          auth: {
            token: faceServiceSocketToken,
          },
        });

        emotionServerSocketRef.current.on('connect', () => {
          console.log('[SingleMessage] ✅ Python expression server connected', {
            url: pythonServerUrl,
            namespace: '/expressions',
            socketId: emotionServerSocketRef.current?.id,
          });
          logDetectionProgress('python_server_connected', {
            url: pythonServerUrl,
            namespace: '/expressions',
            socketId: emotionServerSocketRef.current?.id,
          });
        ...
        emotionServerSocketRef.current.on('connect_error', error => {
          const connectionDescription = (
            error as Error & { description?: unknown }
          ).description;
          console.warn(
            '[SingleMessage] ❌ Failed to connect to Python emotion detection server:',
            {
              url: pythonServerUrl,
          ...
          logDetectionProgress('socket_connect_error', {
            message: error.message,
            hasSocketToken: Boolean(faceServiceSocketToken),
          });
          ...
          if (error.message === 'timeout') {
            console.warn(
              '[SingleMessage] ⏱️ Connection timeout - Render.com server may be cold starting. Will retry...',
            );
          }
        });
        ...
        emotionServerSocketRef.current.on('expression_update', data => {
          ...
          const responseLatencyMs = lastFrameSentAtRef.current
            ...
            : null;
          console.log(
            '[SingleMessage] 📥 Response received from Python expression server',
            {
          ...
          logDetectionProgress('expression_update', {
            label: data?.label,
            confidence: data?.confidence,
            responseLatencyMs,
          });
        ...
        console.error(
          '[SingleMessage] Error initializing emotion server socket:',
          error,
        );
      }
    };
     ...
     */
    const sendRealtimeEmotion = async (
    ...
    const handleEmotionServerResponse = (data: any) => {
      ...
      if (!hasEmotionData) {
        ...
        if (data?.error || data?.message) {
          console.log(
            `[SingleMessage] No face detected: ${data.error || data.message}`,
          );
        }
        return;
      }
      ...
      const emoji = emotionEmojiMap[label] || '😐';
      console.log(
        `[SingleMessage] 🎭 Emotion Detected: ${emoji} ${label} | Category: ${finalEmotion} | Confidence: ${(
          confidence * 100
        ).toFixed(1)}% | Expression: ${dominantExpression || 'none'}`,
      );
      ...
      if (label !== lastMajorityLabelRef.current) {
        ...
        if (
          ...
          try {
            ...
            console.log(
              `[SingleMessage] ⚡ FAST Emotion Change Detected: ${
                previousLabel || 'none'
            ...
            console.error(
              '[SingleMessage] ❌ Error emitting emotion_change:',
              err,
            );
          }
        } else {
      ...
      if (majorityLabel && majorityLabel === label) {
        ...
        if (majorityLabel === lastMajorityLabelRef.current) {
          ...
          console.log(
            `[SingleMessage] ✅ Majority confirmed: ${majorityEmoji} ${majorityLabel} | Window Confidence: ${(
              confidenceApprox * 100
            ).toFixed(1)}% | Window Size: ${windowSize}`,
          );
        }
      }
    };
    ...
    const detectEmotionFromServer = async (base64Image: string) => {
      ...
      detectionStatsRef.current.framesSent += 1;
      const t0 = Date.now();
      ...
      serverRequestTimeoutRef.current = setTimeout(() => {
        console.warn(
          `[SingleMessage] ⏱️ Server request timeout (req ${reqId}) - resetting flag`,
        );
        serverRequestInFlightRef.current = false;
        serverRequestTimeoutRef.current = null;
      ...
      try {
        ...
        logDetectionProgress('frame_sent', {
          reqId,
          payloadBytes: base64Image.length,
        });
      } catch (error: any) {
        const elapsed = Date.now() - t0;
        console.warn(
          `[SingleMessage] Error sending frame to Python server (req ${reqId}, ${elapsed}ms):`,
          error.message,
        );
    ...
    const captureFrameAndSend = async () => {
      ...
      if (!cameraRef.current) {
        console.log(
          '[SingleMessage] ⏸️ Skipping frame - camera ref not available',
        );
        return;
      }
      ...
      if (!emotionServerSocketRef.current?.connected) {
        console.log(
          '[SingleMessage] ⏸️ Skipping frame - emotion server not connected',
        );
        return;
      }
      ...
      if (!isCameraPermissionGranted) {
        console.log(
          '[SingleMessage] ⏸️ Skipping frame - camera permission not granted',
        );
        return;
      }
      ...
      if (!cameraDevice) {
        console.log(
          '[SingleMessage] ⏸️ Skipping frame - camera device not available',
        );
        return;
      }
      ...
      try {
        ...
        console.log(
          `[SingleMessage] 📸 Attempting to capture frame (req ${reqId})`,
        );
        ...
        if (!photo || !photo.uri) {
          console.warn(
            `[SingleMessage] ⚠️ No photo path returned (req ${reqId})`,
          );
          return;
        }

        console.log(
          `[SingleMessage] ✅ Photo captured successfully (req ${reqId}), path: ${photo.uri}`,
        );
        ...
        console.log(
          `[SingleMessage] 📤 Sending frame to server (req ${reqId}), size: ${imageData.length} bytes`,
        );
        ...
        console.error(
          `[SingleMessage] ❌ Error capturing/sending frame (req ${serverRequestSeqRef.current}):`,
          error,
        );
        ...
        if (
          ...
          console.warn(
            '[SingleMessage] ⚠️ Camera configuration error detected',
          );
          // Reset ready state and let it recover
          isCameraReadyRef.current = false;
        } else if (error?.message?.includes('timeout')) {
          console.warn(
            '[SingleMessage] ⚠️ Camera timeout - will retry next interval',
          );
          // Don't stop, just log
        }
      }
    ...
    const detectEmotions = () => {
      ...
      emotionDetectionIntervalRef.current = setInterval(async () => {
        ...
        if (!cameraRef.current) {
          console.log(
            '[SingleMessage] ⏸️ Camera ref not available in interval',
          );
          // Try to recover - check if camera should be available
          if (cameraDevice && isCameraPermissionGranted) {
            console.log(
              '[SingleMessage] 🔄 Camera ref missing but device/permission available - will retry next interval',
            );
          }
          return;
        }
        ...
        if (!isCameraPermissionGranted) {
          ...
          if (timeSinceLastCheck >= PERMISSION_CHECK_COOLDOWN_MS) {
            ...
            console.log(
              '[SingleMessage] ⏸️ Camera permission not granted, re-checking...',
            );
            const permissionOk = await ensureCameraPermission();
            if (!permissionOk) {
              console.log(
                '[SingleMessage] ⏸️ Camera permission still not granted after re-check',
              );
              return;
            }
            // Permission granted, continue with detection
            console.log(
              '[SingleMessage] ✅ Camera permission granted after re-check',
            );
          } else {
        ...
        if (!cameraDevice) {
          console.log(
            '[SingleMessage] ⏸️ Camera device not available in interval',
          );
          return;
        }
        ...
        try {
          ...
          console.error(
            '[SingleMessage] ❌ Error in emotion detection interval:',
            error,
          );
        }
      }, detectionInterval);
    ...
    const startEmotionDetection = async () => {
      ...
      if (!shouldUseCamera) {
        console.log(
          '[SingleMessage] ⏸️ Skipping emotion detection - screen not focused or app in background',
        );
        return;
      }
      ...
      console.log(
        '[SingleMessage] 📷 Camera permission granted, device available:',
        cameraDevice?.id,
      );
      logDetectionProgress('camera_prerequisites_ok');
      ...
      logDetectionProgress('camera_wait_started');
      ...
      for (let i = 0; i < maxChecks; i++) {
        ...
        if (cameraRef.current && isCameraReadyRef.current) {
          cameraReady = true;
          console.log(
            '[SingleMessage] ✅ Camera is ready after',
            (i + 1) * checkInterval,
            'ms',
          );
          break;
        ...
        if (i > 0 && i % 4 === 0) {
          console.log(
            `[SingleMessage] ⏳ Still waiting for camera... (${
              (i + 1) * checkInterval
            }ms)`,
            {
      ...
      if (!cameraReady) {
        console.warn(
          '[SingleMessage] ⚠️ Camera not ready after waiting, but continuing anyway',
        );
        ...
        logDetectionProgress('camera_wait_timeout');
        // Try to mark as ready anyway if ref exists (might work)
        if (cameraRef.current) {
          console.log(
            '[SingleMessage] 🔄 Attempting to force camera ready state',
          );
          isCameraReadyRef.current = true;
        }
      }
      ...
      while (
        ...
        if (connectionAttempts % 5 === 0) {
          console.log(
            `[SingleMessage] ⏳ Waiting for emotion server connection... (${connectionAttempts}s)`,
          );
        }
      }
      ...
      if (!emotionServerSocketRef.current?.connected) {
        console.warn(
          '[SingleMessage] ⚠️ Emotion server not connected after initialization. Connection may still be establishing in background.',
        );
      ...
      setTimeout(() => {
        ...
        if (cameraRef.current && cameraDevice && isCameraPermissionGranted) {
          if (!isCameraReadyRef.current) {
            console.log(
              '[SingleMessage] 🔄 Camera ref exists but not marked ready - marking now',
            );
            isCameraReadyRef.current = true;
          }
          ...
          console.warn(
            '[SingleMessage] ⚠️ Camera not ready after delay, starting detection anyway',
          );
          logDetectionProgress('detection_started_without_camera_ready');
          detectEmotions();
        }
  ...
  const handleCameraRef = React.useCallback(
    (ref: CameraView | null) => {
      ...
      if (ref) {
        ...
        logDetectionProgress('camera_ref_attached');
        ...
        cameraSetupTimeoutRef.current = setTimeout(() => {
          ...
          if (cameraRef.current && cameraDevice) {
            ...
            logDetectionProgress('camera_marked_ready');
          } else {
            console.warn(
              '[SingleMessage] ⚠️ Camera ref or device lost during initialization',
            );
            isCameraReadyRef.current = false;
          }
          cameraSetupTimeoutRef.current = null;
        ...
        logDetectionProgress('camera_ref_detached');
        isCameraReadyRef.current = false;
      }
    },
    [cameraDevice],
  ...
  useEffect(() => {
    if (isCallActive || !cameraDevice || !isCameraPermissionGranted) {
      return;
    }
    ...
    const checkInterval = setInterval(() => {
      if (
        ...
        console.log(
          '[SingleMessage] 🔄 Camera conditions met but not marked ready - marking now',
        );
        isCameraReadyRef.current = true;
      }
  ...
  }, [isCallActive, cameraDevice, isCameraPermissionGranted]);
    ...
    React.useCallback(() => {
      ...
      if (shareFaceModeEnabled) {
        ensureCameraPermission().catch(err => {
          console.warn(
            '[SingleMessage] Error checking camera permission on focus:',
            err,
          );
        });
      }
        ...
        setMessages(prev =>
          upsertConfirmedMessage(
            prev,
            {
              ...
              timestamp: new Date(
                response.updatedMessage.timestamp || Date.now(),
              ),
              tempId,
            },
            tempId,
  ...
  sendVoiceMessageRef.current = (voiceUrl: string, duration: number) => {
    sendMessage({
      message: '',
      attachment: voiceUrl,
      messageType: 'audio',
  ...
  const handleInputChange = (value: string) => {
    ...
    const showTyping =
      settings.showIsTyping !== false &&
      settings.showTyping !== false &&
      settings.typingIndicators !== false;
    if (!showTyping) return;

    if (value.trim().length > 0) {
      const now = Date.now();
      const shouldEmit =
        !isTypingOutgoingRef.current || now - lastTypingEmitRef.current > 400;
      if (shouldEmit) {
        const roomId =
          room ||
  ...
  const handleMessageLongPress = (message: Message, event: any) => {
    ...
    setIsReactedByMe(
      normalizeMessageReactions(message?.reacts).some(
        reaction => reactionProfileId(reaction) === String(myProfile?._id),
      ),
    );
  ...
  const likeOrUnlikeMessage = (reactionType = '👍') => {
    ...
    const shouldRemove = Boolean(
      currentReaction && currentReaction.type === reactionType,
    );
    ...
    setMessages(prev =>
      prev.map(m => (m._id === messageId ? { ...m, reacts: nextReacts } : m)),
    );
    setSelectedMessage(message =>
      message ? { ...message, reacts: nextReacts } : message,
    );

    const rollback = () => {
      setMessages(prev =>
        prev.map(m =>
          m._id === messageId ? { ...m, reacts: previousReacts } : m,
        ),
      );
      setSelectedMessage(message =>
        message ? { ...message, reacts: previousReacts } : message,
      );
  ...
  const viewImage = () => {
    if (
      ...
      isValidImageUrl(selectedMessage.attachment)
    ) {
  ...
  const pasteImageFromClipboard = async () => {
    ...
    try {
      ...
      const base64 = clipboardImage.data.replace(
      ...
      Alert.alert(
        'Paste failed',
        'Could not paste an image from the clipboard.',
      );
    }
  };
  ...
  const composerIconBtn = (active?: boolean) => ({
    ...
    backgroundColor: active
      ? `${chatTheme.colors.accent}33`
      : chatTheme.colors.recvBg,
  ...
  const loadOldMessages = useCallback(async () => {
    ...
    try {
      ...
      const existingIds = new Set(
        messagesRef.current.map(msg => String(msg._id)),
      );
      const uniqueOlder = older.filter(
        msg => !existingIds.has(String(msg._id)),
      );
  ...
  const handleScroll = useCallback(
    (event: any) => {
      ...
      if (
        contentOffset.y >=
        maxScroll * (1 - LOAD_OLDER_SCROLL_PERCENT / 100)
      ) {
        loadOldMessages();
      }
    },
  ...
  const renderMessage = ({ item }: { item: Message }) => {
    ...
    return (
      <Swipeable
        ...
        renderLeftActions={() =>
          !isMyMessage ? renderLeftReplyAction() : null
        }
      ...
      >
        ...
        >
          <View
            style={{
              marginTop:
                Array.isArray(item.reacts) && item.reacts.length > 0 ? 18 : 0,
              marginBottom:
                Array.isArray(item.reacts) && item.reacts.length > 0 ? 16 : 8,
              marginHorizontal: 12,
              flexDirection: 'row',
              alignItems: 'flex-end',
              justifyContent: isMyMessage ? 'flex-end' : 'flex-start',
              overflow: 'visible',
              zIndex:
                Array.isArray(item.reacts) && item.reacts.length > 0 ? 10 : 0,
              elevation:
                Array.isArray(item.reacts) && item.reacts.length > 0 ? 10 : 0,
            }}
          >
            ...
            >
                ...
                style={{
                  ...
                  borderColor:
                    highlightedMessageId === item._id
                      ...
                      : isMyMessage
                      ? chatTheme.colors.sentBorder
                      : chatTheme.colors.recvBorder,
                  position: 'relative',
                  overflow: 'visible',
                  zIndex:
                    Array.isArray(item.reacts) && item.reacts.length > 0
                      ? 11
                      : 0,
                  elevation:
                    Array.isArray(item.reacts) && item.reacts.length > 0
              ...
              >
                {item.parent ? (
                  <TouchableOpacity
                    onPress={() =>
                      item.parent?._id && scrollToMessage(item.parent._id)
                    }
                    activeOpacity={0.7}
                    style={{
                      marginBottom: 8,
                  ...
                  >
                    {typeof item.parent.attachment === 'string' &&
                    isValidImageUrl(item.parent.attachment) ? (
                      <Image
                  ...
                  >
                    ...
                    <Slider
                      ...
                      maximumValue={Math.max(
                        1,
                        Math.floor(
                          playingProgress[item._id]?.duration ||
                            item.duration ||
                            0,
                        ),
                      )}
                      value={Math.floor(
                        playingProgress[item._id]?.current || 0,
                      )}
                      ...
                      maximumTrackTintColor={
                        isMyMessage
                          ? `${chatTheme.colors.sentText}59`
                          : `${chatTheme.colors.recvText}59`
                      }
                      thumbTintColor={
                        isMyMessage
                          ? chatTheme.colors.sentText
                    ...
                    >
                      {formatSecs(playingProgress[item._id]?.current || 0)} /{' '}
                      {formatSecs(
                        playingProgress[item._id]?.duration ||
                          item.duration ||
                          0,
                  ...
                  {isMyMessage && item.sendFailed ? (
                    <TouchableOpacity
                      onPress={() => {
                        setMessages(prev =>
                          prev.filter(
                            m => m._id !== item._id && m.tempId !== item.tempId,
                          ),
                        );
                ...
                {normalizeMessageReactions(item.reacts).length > 0 ? (
                  ...
                  >
                    {Object.entries(
                      normalizeMessageReactions(item.reacts).reduce<
  ...
  const renderLoadingOldMessages = () => {
    ...
    return (
      ...
      >
        <ActivityIndicator
          size="small"
          color={chatTheme.colors.accent || '#ffffff'}
        />
      </View>
    );
  };
                  ...
                  style={{
                    ...
                    backgroundColor:
                      Platform.OS === 'ios'
                        ...
                        : themeColors.surface.header ||
                          themeColors.background.primary ||
                          (isDarkMode ? '#242526' : '#FFFFFF'),
                    borderRadius: 16,
                    paddingVertical: 8,
                    minWidth: 200,
                    shadowColor: '#000',
                  ...
                  >
                    ...
                    >
                      <Icon
                        name="call"
                        size={22}
                        color={themeColors.status.success || '#4CAF50'}
                      />
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text
        ...
        {isInitialLoading ? (
          ...
          >
            ...
            <FlatList
              ...
              keyExtractor={item =>
                item._id || item.tempId || item.timestamp?.toString()
              }
            ...
            >
              ...
              >
                {QUICK_REACTION_PRESETS.slice(0, 8).map(emoji => {
                  const selected = normalizeMessageReactions(
                    selectedMessage?.reacts,
                  ).some(
                    reaction =>
                      reactionProfileId(reaction) === String(myProfile?._id) &&
                      reaction.type === emoji,
                  );
                  return (
                    <TouchableOpacity
              ...
              >
                <Icon
                  name={isReactedByMe ? 'thumb-down' : 'thumb-up'}
                  size={20}
                  color={themeColors.text.primary}
                />
                <Text
              ...
              >
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    Alert.alert(
                      'Media',
                      'View shared media feature coming soon!',
                    );
                  }}
                >
                  <View
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    Alert.alert(
                      'Mute',
                      'Mute conversation feature coming soon!',
                    );
                  }}
                >
                  <View
                    ...
                    Alert.alert('Report', `Report ${friend?.fullName}?`, [
                      { text: 'Cancel', style: 'cancel' },
                      {
                        ...
                        onPress: () => {
                          Alert.alert(
                            'Report',
                            'Report user feature coming soon!',
                          );
                        },
                      },
                    ]);
            ...
            >
              {replyingTo ? (
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    <Text
                      style={{ color: chatTheme.colors.meta, fontSize: 11 }}
                    >
                      Replying to{' '}
                    ...
                    >
                      {typeof replyingTo.attachment === 'string' &&
                        isValidImageUrl(replyingTo.attachment) && (
                          <Image
                            source={{ uri: replyingTo.attachment as string }}
                            style={{
                              width: 28,
                              height: 28,
                              borderRadius: 4,
              ...
              {isUploading || pendingAttachmentLocal || pendingAttachment ? (
                ...
                >
                  {pendingAttachmentLocal || pendingAttachment ? (
                    <Image
                      source={{
                        uri: pendingAttachmentLocal || pendingAttachment || '',
                      }}
                      style={{
                        width: 48,
                        height: 48,
                  ...
                  <View style={{ flex: 1 }}>
                    {isUploading ? (
                      <View
                        style={{
                          height: 6,
                          backgroundColor: chatTheme.colors.recvBorder,
                          borderRadius: 3,
                          overflow: 'hidden',
                        }}
                      >
              ...
              {isRecording ? (
                ...
                >
                  ...
                  <Text
                    style={{ color: chatTheme.colors.meta, marginRight: 10 }}
                  >
                    {recordTime}
                  </Text>
                  <View
                ...
                {inputText.trim() || pendingAttachment ? (
                  ...
                  >
                    <Icon
                      ...
                      style={{ marginRight: -2 }}
                      size={18}
                      color={isUploading ? chatTheme.colors.meta : '#041018'}
                    />
                  </TouchableOpacity>
                ) : (
                  <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <View>
                      <TouchableOpacity
                        ...
                        style={composerIconBtn(
                          isRecording || isTranscribing || showMicMenu,
                        )}
                      >
                        {isUploadingAudio ? (
                          ...
                          <Icon
                            name={
                              isRecording || isTranscribing ? 'stop' : 'mic'
                            }
                            size={20}
                            color={
                ...
                <View style={{ marginTop: 12 }}>
                  ...
                  >
                    {[
                      ...
                      {
                        key: 'live',
                        label: isLiveVoiceActive
                          ? 'Stop'
                          : isLiveVoiceConnecting
                          ? 'Wait'
                          : 'Live',
                        icon: isLiveVoiceActive ? 'phone-disabled' : 'headset',
                        onPress: () => {
                          if (
                            isLiveVoiceConnecting ||
                            isRecording ||
                      ...
                      },
                      {
                        ...
                        label: 'Edit',
                        icon: 'edit',
                        onPress: () => setEditReactionOpen(v => !v),
                      },
                    ].map(item => (
                      <TouchableOpacity
                        key={item.key}
                      ...
                      {QUICK_REACTION_PRESETS.map(emoji => (
                        <TouchableOpacity
                          ...
                          style={{
                            ...
                            marginRight: 6,
                            backgroundColor:
                              chatAppearance.actionEmoji === emoji
                                ? `${chatTheme.colors.accent}33`
                                : chatTheme.colors.recvBg,
                            borderWidth: 1,
                            borderColor:
        ...
        >
          <CameraView
            ...
            onMountError={error => {
              isCameraReadyRef.current = false;
              console.error(
                '[SingleMessage] ❌ Expo camera mount error:',
                error,
              );
              logDetectionProgress('expo_camera_mount_error', {
